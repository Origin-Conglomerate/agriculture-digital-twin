...
const Soil04 = () => {
  ...
  const getSoilMetrics = () => {
    ...
    const { soilMoistureL1, soilMoistureL2, soilTemprature, leafWetness } = soilData.iotData;
    return [
      {
        id: 'moisture1',
        name: 'Layer 1 Moisture',
        value: soilMoistureL1.toFixed(2),
        unit: 'cbar',
        icon: Droplets,
        analyticsKey: 'soilMoistureL1Analytics',
        status: soilData.analytics?.soilMoistureL1Analytics?.status,
        description: soilData.analytics?.soilMoistureL1Analytics?.description,
        trend: soilMoistureL1 > 30 ? 'increasing' : 'stable',
        alert: soilMoistureL1 < 10 ? 'Low moisture detected' : null
      },
      {
        id: 'moisture2',
        name: 'Layer 2 Moisture',
        value: soilMoistureL2.toFixed(2),
        unit: 'cbar',
        icon: Sprout,
        analyticsKey: 'soilMoistureL2Analytics',
        status: soilData.analytics?.soilMoistureL2Analytics?.status,
        description: soilData.analytics?.soilMoistureL2Analytics?.description,
        trend: soilMoistureL2 > 35 ? 'increasing' : 'stable',
        alert: soilMoistureL2 < 15 ? 'Low moisture detected' : null
      },
      {
        id: 'temperature',
        name: 'Soil Temperature',
        value: soilTemprature,
        unit: '°C',
        icon: Thermometer,
        analyticsKey: 'soilTemperatureAnalytics',
        status: soilData.analytics?.soilTemperatureAnalytics?.status,
        description: soilData.analytics?.soilTemperatureAnalytics?.description,
        trend: soilTemprature > 25 ? 'warning' : 'optimal',
        alert: soilTemprature > 30 ? 'High temperature alert' : null
      },
      {
        id: 'wetness',
        name: 'Leaf Wetness',
  ...
  const handleRefresh = async () => {
    setIsLoading(true);
    try {
      const response = await GET(`${import.meta.env.VITE_API_URL}/api/v1/analytics`, token);
      if (response?.data?.analytics) {
        setSoilData(response.data.analytics);
        setLastUpdated(new Date());
      }
