...
const useWeatherAnalytics = (weatherData) => {
  ...
  useEffect(() => {
    if (weatherData && weatherData.analytics) {
      setAnalytics({
        temperature: {
          status: weatherData.analytics.airTempAnalytics?.status || 'Normal',
          description: weatherData.analytics.airTempAnalytics?.description || 'Temperature is within expected range'
        },
        humidity: {
          status: weatherData.analytics.airHumidityAnalytics?.status || 'Normal',
          description: weatherData.analytics.airHumidityAnalytics?.description || 'Humidity levels are optimal'
        },
        rainfall: {
        ...
        pressure: {
          status: weatherData.analytics.airPressureAnalytics?.status || 'Normal',
          description: weatherData.analytics.airPressureAnalytics?.description || 'Pressure systems are stable'
        }
      });
    }
  }, [weatherData]);
...
const Weather05 = () => {
  ...
  const getWeatherMetrics = () => {
    ...
    const { airTemp, airHumidity, rainFall, airPressure } = weatherData.iotData;
    ...
    return {
      ...
      humidity: {
        ...
        unit: '%',
        icon: Droplets,
        trend: airHumidity.trend || 'stable',
      ...
      pressure: {
        value: airPressure.toFixed(2),
        unit: 'hPa',
        icon: Gauge,
        trend: airPressure.trend || 'stable',
